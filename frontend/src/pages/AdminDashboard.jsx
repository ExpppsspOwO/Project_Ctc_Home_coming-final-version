...
const MenuList = ({ isCollapsed, activeTab, onMenuClick, onLogout, onNavigate }) => (
  <nav className="flex flex-col w-full mt-4 pb-4">
    ...
    <div className="mt-auto pt-4 pb-2 border-t border-gray-100 dark:border-slate-800">
      <button onClick={() => onNavigate('/')} className="relative w-full flex items-center gap-4 px-6 py-4 text-gray-600 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800 hover:text-gray-900 dark:hover:text-white border-l-4 border-transparent transition-all duration-200 group">
...
const AdminDashboard = () => {
  ...
  const handleMenuClick = (tabId) => {
    ...
    if (isTablet) setIsDesktopSidebarOpen(false);
  };
  ...
  return (
    ...
    <div className="h-screen bg-gray-100 dark:bg-slate-950 flex overflow-hidden transition-colors duration-300">
      
      {/* Mobile Navbar */}
      <div className="md:hidden bg-white dark:bg-slate-900 shadow-sm border-b border-gray-200 dark:border-slate-800 px-4 py-3 flex justify-between items-center fixed top-0 left-0 right-0 z-40 transition-colors">
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsMobileMenuOpen(false)} className="fixed inset-0 bg-black/20 dark:bg-black/50 z-50 md:hidden backdrop-blur-[2px]" />
            <motion.aside initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} className="fixed inset-y-0 left-0 w-72 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl shadow-2xl z-50 md:hidden flex flex-col border-r border-gray-100 dark:border-slate-800">
              <div className="p-6 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-blue-500 rounded-lg flex items-center justify-center"><span className="text-white font-bold">CTC</span></div>
      ...
      <motion.aside
        ...
        className={`hidden md:flex flex-col border-r border-gray-200 dark:border-slate-800 h-full fixed top-0 left-0 z-30 shadow-sm overflow-hidden bg-white dark:bg-slate-900 transition-colors duration-300`}
      >
        <div className="h-20 flex items-center justify-center border-b border-gray-100 dark:border-slate-800 shrink-0 px-6">
          {isDesktopSidebarOpen ? (
            <div className="flex items-center gap-3 w-full">
                <div className="w-10 h-10 bg-blue-500 rounded-lg flex items-center justify-center shrink-0"><span className="text-white font-bold">CTC</span></div>
      ...
      <motion.main
        initial={false}
        animate={{ marginLeft: isMobile ? 0 : (isTablet ? 80 : (isDesktopSidebarOpen ? 280 : 80)) }}
        transition={{ duration: 0.3, ease: 'easeInOut' }}
        className="flex-1 overflow-y-auto mt-14 md:mt-0 min-h-screen max-md:!ml-0"
      >
        <div className="p-4 md:p-8 w-full max-w-[1920px]">
          ...
          <div className="min-h-[600px] w-full">
            ...
            {activeTab === 'souvenirs' && <SouvenirManagement />}
            {activeTab === 'orders' && <SouvenirOrderManager />}
            {activeTab === 'users' && <UserManagement />}
            {activeTab === 'zones' && <ZoneGenerator />}
