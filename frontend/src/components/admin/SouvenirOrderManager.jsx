...
const SouvenirOrderManager = () => {
  ...
  // ✅ เพิ่มตัวเลือกสถานะ (เหมือน BookingManager)
  ...
  const [actionModal, setActionModal] = useState({
    isOpen: false,
    type: null, // 'verify', 'pickup', 'cancel'
    order: null
  });
  ...
  const handleUpdateStatus = async () => {
    ...
    if (type === 'verify') {
        ...
        // ✅ ส่งทั้ง status (ตามที่เลือก) และ remark
  ...
  const getFilteredOrders = () => {
    ...
    if (activeTab === 'pending') {
        // ✅ เพิ่ม payment_failed เข้ามาในแท็บนี้ด้วย
  ...
  const openActionModal = (type, order) => {
    setCancelReason('');
    setCancelType('cancelled'); // Reset ค่าเริ่มต้น
    ...
    <div className="w-full pb-20 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-200 dark:border-slate-700 pb-1">
         <div className="flex gap-1 overflow-x-auto w-full sm:w-auto pb-2 sm:pb-0 scrollbar-hide">
            <button 
              ...
              className={`px-4 py-2 rounded-t-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap
                ${activeTab === 'pending' 
            ...
            >
              <Clock size={16} /> รอตรวจสอบ ({orders.filter(o => o.status === 'pending' || o.status === 'waiting_verify' || o.status === 'payment_failed').length})
            </button>
            <button 
              ...
              className={`px-4 py-2 rounded-t-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap
                ${activeTab === 'paid' 
                  ...
                  : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
            >
              <ShoppingBag size={16} /> รอรับสินค้า ({orders.filter(o => o.status === 'paid').length})
            </button>
            <button 
              onClick={() => setActiveTab('history')}
              className={`px-4 py-2 rounded-t-lg text-sm font-bold flex items-center gap-2 transition-all whitespace-nowrap
                ${activeTab === 'history' 
                  ? 'bg-white dark:bg-slate-900 text-gray-700 dark:text-gray-200 border-b-2 border-gray-500 dark:border-gray-400' 
                  : 'text-gray-500 dark:text-slate-400 hover:bg-gray-50 dark:hover:bg-slate-800'}`}
            >
              <FileText size={16} /> ประวัติ ({orders.filter(o => ['completed', 'cancelled'].includes(o.status)).length})
            </button>
         </div>
                           ...
                           <td className="p-4 text-center align-top">
                              <div className="flex items-center justify-center gap-1.5">
                                 ...
                                 {(order.status === 'pending' || order.status === 'waiting_verify' || order.status === 'payment_failed') && (
                                     <button 
                                        onClick={(e) => { e.stopPropagation(); openActionModal('verify', order); }} 
                                        title="อนุมัติ (Verify)" 
                                        className="p-1.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-lg border border-emerald-100 dark:border-emerald-800 hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition"
                                     >
                                        <Check size={16} />
                                     </button>
                                 )}
                                 ...
                                 {order.status === 'paid' && (
                                     <button 
                                        onClick={(e) => { e.stopPropagation(); openActionModal('pickup', order); }} 
                                        title="ยืนยันการรับของ" 
                                        className="p-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-lg border border-blue-100 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition"
                                     >
                                        <UserCheck size={16} />
                                     </button>
                                 )}
                                 ...
                                 {!['completed', 'cancelled'].includes(order.status) && (
                                     <button 
                                        onClick={(e) => { e.stopPropagation(); openActionModal('cancel', order); }} 
                                        title="ยกเลิก/ปฏิเสธ" 
                                        className="p-1.5 bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 rounded-lg border border-red-100 dark:border-red-800 hover:bg-red-100 dark:hover:bg-red-900/40 transition"
                                     >
                                        <X size={16} />
                                     </button>
                                 )}
      ...
      <AnimatePresence>
        {isDeleteMode && selectedIds.length > 0 && (
          <motion.div
            ...
            className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-gray-900 dark:bg-slate-800 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-6 border border-gray-700"
          >
            <span className="text-sm font-bold">{selectedIds.length} รายการที่เลือก</span>
            <div className="h-4 w-[1px] bg-gray-600"></div>
            <button onClick={() => setShowBulkDeleteModal(true)} className="flex items-center gap-2 text-red-400 hover:text-red-300 font-bold text-sm">
         ...
         {actionModal.isOpen && (
            ...
            >
                <motion.div 
                    ...
                    className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-sm p-6 relative overflow-hidden border border-gray-100 dark:border-slate-800"
                >
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4
                        ...
                        {actionModal.type === 'cancel' ? <AlertTriangle size={32} /> : 
                         actionModal.type === 'pickup' ? <UserCheck size={32} /> : <Check size={32} />}
                    </div>

                    <h3 className="text-xl font-bold text-center mb-2 text-gray-800 dark:text-white">
                        {actionModal.type === 'verify' && 'ยืนยันการชำระเงิน?'}
                        {actionModal.type === 'pickup' && 'ยืนยันลูกค้าได้รับของแล้ว?'}
                        {actionModal.type === 'cancel' && 'จัดการออเดอร์'}
                    </h3>
                    
                    {actionModal.type === 'cancel' && (
                        <div className="space-y-4 text-left">
                            <p className="text-sm text-gray-600 dark:text-slate-400 text-center">
                                เลือกการดำเนินการสำหรับ Order #{actionModal.order?._id.slice(-6).toUpperCase()}
                            </p>
                            
                            {/* ✅ Radio Button เลือกประเภทการยกเลิก */}
                            <div className="flex flex-col gap-2">
                                <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${cancelType === 'cancelled' ? 'bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800' : 'border-gray-200 dark:border-slate-700'}`}>
                                    ...
                                    <div>
                                        ...
                                        <span className="block text-xs text-gray-500 dark:text-slate-400">ออเดอร์จะถูกยกเลิก ลูกค้าต้องสั่งใหม่</span>
                                    </div>
                                </label>
                                ...
                                <label className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${cancelType === 'payment_failed' ? 'bg-amber-50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800' : 'border-gray-200 dark:border-slate-700'}`}>
                                    ...
                                    <div>
                                        ...
                                        <span className="block text-xs text-gray-500 dark:text-slate-400">ออเดอร์ยังอยู่ ลูกค้าสามารถส่งสลิปใหม่ได้</span>
                                    </div>
                                </label>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    placeholder="เช่น สลิปไม่ชัด, ยอดเงินไม่ครบ..."
                                    className="w-full p-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl text-sm focus:ring-2 focus:ring-red-500 outline-none transition resize-none"
                                    rows="3"
                                />
                            </div>
                        </div>
                    ...
                    <div className="flex gap-3 mt-6">
                        <button onClick={closeActionModal} className="flex-1 py-2 bg-gray-100 dark:bg-slate-800 rounded-xl font-bold text-gray-600 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700">ยกเลิก</button>
                        <button onClick={handleUpdateStatus} className={`flex-1 py-2 rounded-xl font-bold text-white shadow-lg transition active:scale-95
                            ${actionModal.type === 'cancel' ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'}`}>
                            ยืนยัน
                        </button>
                    </div>
                </motion.div>
      ...
      <AnimatePresence>
        {showBulkDeleteModal && (
          <motion.div
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 bg-red-900/40 flex items-center justify-center z-[60] backdrop-blur-sm p-4"
            onClick={() => setShowBulkDeleteModal(false)}
          >
            <motion.div
              initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center border border-red-100 dark:border-red-900"
              onClick={e => e.stopPropagation()}
            >
               <h3 className="text-xl font-bold text-red-600 dark:text-red-500 mb-2">ยืนยันลบ {selectedIds.length} รายการ?</h3>
               <p className="text-gray-500 dark:text-slate-400 text-sm mb-6">การกระทำนี้ไม่สามารถกู้คืนได้</p>
               <div className="flex gap-3">
                  <button onClick={() => setShowBulkDeleteModal(false)} className="flex-1 py-2 bg-gray-100 dark:bg-slate-800 rounded-xl font-bold text-gray-600 dark:text-slate-300">ยกเลิก</button>
                  <button onClick={confirmBulkDelete} className="flex-1 py-2 bg-red-600 rounded-xl font-bold text-white shadow-lg hover:bg-red-700">ลบทิ้ง</button>
               </div>
            </motion.div>
          </motion.div>
        )}
        ...
        {previewImage && (
          ...
          >
            <div className="relative max-w-full max-h-[90vh]">
               ...
               <img src={previewImage} alt="Slip" className="max-w-full max-h-[85vh] rounded-lg bg-gray-200 dark:bg-slate-800" />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
